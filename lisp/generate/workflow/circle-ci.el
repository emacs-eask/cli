        ...
        (with-current-buffer (find-file filename)
          ...
          (when (search-forward "{ EMACS_VERSION }" nil t)
            ...
            (when (version<= minimum-version "26.1")
              (eask-generate-workflow-circle-ci--insert-jobs "26"))
            (when (version<= minimum-version "27.1")
              (insert "\n")
              (eask-generate-workflow-circle-ci--insert-jobs "27"))
            (when (version<= minimum-version "28.1")
              (insert "\n")
              (eask-generate-workflow-circle-ci--insert-jobs "28"))
            (when (version<= minimum-version "29.1")
              (insert "\n")
              (eask-generate-workflow-circle-ci--insert-jobs "29"))
            (when (version<= minimum-version "30.1")
              (insert "\n")
              (eask-generate-workflow-circle-ci--insert-jobs "30"))
            (when (version<= minimum-version "31")
              (insert "\n")
              (eask-generate-workflow-circle-ci--insert-jobs "master")))
          ;; Config matrix
