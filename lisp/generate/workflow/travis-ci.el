          ...
          (when (search-forward "{ EMACS_VERSION }" nil t)
            ...
            (let ((column (current-column)))
              ...
              (when (version<= minimum-version "26.1")
                (insert (spaces-string column) "- EMACS_CI=emacs-26-3" "\n"))
              (when (version<= minimum-version "27.1")
                (insert (spaces-string column) "- EMACS_CI=emacs-27-2" "\n"))
              (when (version<= minimum-version "28.1")
                (insert (spaces-string column) "- EMACS_CI=emacs-28-2" "\n"))
              (when (version<= minimum-version "29.1")
                (insert (spaces-string column) "- EMACS_CI=emacs-snapshot"))))
          (save-buffer))
        "done ✓")
