...
```js
export const command = ['archives', 'sources'];  // alias to sources
...
- **export const command** is the argument pattern, but it also accepts alias (array)
- **export const desc** is the command description
- **export const handler** is an asynchronous function that handles command execution
...
For **local** options, please use `export const builder` and specify under its
command file.
