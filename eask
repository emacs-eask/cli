...
import { TITLE_PROXY_OPTION } from './src/env.js';
...
handler
  ...
  .scriptName('')
  .epilogue(epilogue)
  .commandDir(path.join(__dirname, './cmds/core/'))
  .commandDir(path.join(__dirname, './cmds/util/'))
  .command({
    command: '*',
    handler() { handler.showHelp(); }
  })
